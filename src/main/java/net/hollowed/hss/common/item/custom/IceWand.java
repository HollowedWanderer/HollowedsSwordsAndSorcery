...
public class IceWand extends SwordItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level p_40672_, Player p_40673_, InteractionHand p_40674_) {
        ItemStack itemstack = p_40673_.getItemInHand(p_40674_);
            IceWandCharging.execute(p_40672_, p_40673_, itemstack);
    ...
    public void releaseUsing(ItemStack stack, Level world, LivingEntity entity, int p_40670_) {
        if (entity instanceof Player player) {
            ...
            if (charged) {
                ...
                IceWandClick.execute(world, player, stack);
                charged = false;
            } else {
                IceWandFail.execute(world, player, stack);
            }
        }
    }
    @Override
    public void appendHoverText(ItemStack p_41421_, @Nullable Level level, List<Component> components, TooltipFlag tooltipFlag) {
        components.add(Component.literal("The Iceologer's magic lies within this wand").withStyle(ChatFormatting.AQUA));
    }
}
